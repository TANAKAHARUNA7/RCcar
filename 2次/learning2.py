...
# 데이터 경로 및 설정
DATASET_DIR = r'C:\data\processed_dataset2'
BATCH_SIZE = 64
EPOCHS = 30
...
class CNNModel(nn.Module):
    def __init__(self, num_classes):
        ...
        self.dropout = nn.Dropout(0.3)  # Dropout 확률 조정 (0.6)
    ...
    def forward(self, x):
        ...
        x = x.view(-1, 64 * 8 * 8)
        x = self.dropout(torch.relu(self.fc1(x)))  # Dropout 적용
        x = self.fc2(x)
        return x
...
optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE, weight_decay=1e-5)  # Weight Decay 강화 (1e-3)
